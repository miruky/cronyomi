...
入力欄にcron式を打つか、プリセットのチップを選ぶと、3つの情報が同時に出ます。
...
曜日は `0` と `7` のどちらも日曜として扱います。日(第3フィールド)と曜日(第5フィールド)を同時に指定した場合は、cronの仕様どおりどちらか一方に一致すれば実行とみなします。
...
- `src/cron.ts` — cron式のパーサ。範囲・ステップ・名前・別表記を展開する
